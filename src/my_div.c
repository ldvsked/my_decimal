...
int my_div(my_decimal value_1, my_decimal value_2, my_decimal *result) {
  ...
  my_decimal man1 = value_1;
  my_decimal man2 = value_2;
  man1.bits[3] = 0;
  man2.bits[3] = 0;
  ...
  while (!is_full_value_zero(remainder) && res_scale < 28 && code == S21_OK) {
    my_decimal next_val = temp_res;
    if (mul_by_10(&next_val)) break;

    my_decimal rem_x10 = remainder;
    mul_by_10(&rem_x10);

    my_decimal part_q, part_r;
  ...
  if (code == S21_OK) {
    ...
    if (!is_full_value_zero(remainder)) {
      ...
      if (my_is_greater(remainder, difference)) {
        needs_rounding = 1;
      } else if (my_is_equal(remainder, difference)) {
